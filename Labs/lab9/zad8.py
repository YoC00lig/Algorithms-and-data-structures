# Zadanie 6. (dwóch kierowców) Dana jest mapa kraju w postaci grafu G = (V, E), gdzie wierzchołki to miasta a
# krawędzie to drogi łączące miasta. Dla każdej drogi znana jest jej długość (wyrażona w kilometrach
...
# Proszę zapropnować algorytm, który wskazuje taką trasę (oraz osobę, która ma prowadzić pierwsza), żeby Alicja
# przejechała jak najmniej kilometrów. Algorytm powinien być jak najszybszy (ale przede wszystkim poprawny).
...
def dijkstra(G: 'graph represented by adjacency lists', s: 'source', t: 'target'):
    ...
    # the last person who drives is Alice 0 or Bob 1>)
    ...
    while not pq.empty():
        ...
        if min_dist < weights[u][driver]:
            ...
            for v, weight in G[u]:
                ...
                elif driver == 1 and  weights[v][0] == inf:
